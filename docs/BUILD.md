...
The `build` command publishes a self-contained `win-x64` application, adds the update worker, creates `dist\BlindTerm-v1.0.0.zip`, creates `dist\BlindTerm-Setup-v1.0.0.exe`, and writes `dist\BlindTerm-update.json` with SHA-256 values.
