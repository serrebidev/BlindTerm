...
The `build` command publishes a self-contained `win-x64` application, adds the update worker, creates `dist\BlindTerm-v0.1.0.zip`, creates `dist\BlindTerm-Setup-v0.1.0.exe`, and writes `dist\BlindTerm-update.json` with SHA-256 values.
