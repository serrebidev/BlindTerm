...
The `build` command publishes a self-contained `win-x64` application, adds the update worker, creates `dist\BlindTerm-v0.2.2.zip`, creates `dist\BlindTerm-Setup-v0.2.2.exe`, and writes `dist\BlindTerm-update.json` with SHA-256 values.
